...
extern "C" char *get_iOS_RomPath();
extern "C" char *get_iOS_SamplePath();
//
...
void parse_cmdline (int argc, char **argv, int game_index)
{
	 ...
	 */
	// no jailbreak - Les Bird
	nvdir = strdup(get_documents_path("nvram"));
	...
	artworkdir = strdup(get_documents_path("artwork"));
	cheatdir = strdup(get_documents_path("cheat"));
	//
