...
// c interfaces to the obj-c functions - Les Bird
char *get_iOS_RomPath()
{
	return [Helper getRomPath];
}

char *get_iOS_SamplePath()
{
	return [Helper getSamplePath];
}

char *get_iOS_BundlePath()
{
	return [Helper getBundlePath];
}
//
...
+ (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
  ...
  else
  {
     ...
     EmulatorController *eC = (EmulatorController *)[inqViewControl presentingViewController];	
     [eC endMenu];
...
// Les Bird
// documents folder (roms, highscores, config settings, nvram)
+ (char *)getRomPath
{
	NSArray *paths = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES);
	const char *userPath = [[paths objectAtIndex:0] cStringUsingEncoding:NSASCIIStringEncoding];
	
	return (char *)userPath;
}

// app bundle path (samples folder)
// all samples are part of the app bundle in order to prevent interferring with roms of the same name in the documents folder
// and since you can't copy folders to the documents folder in iTunes, no choice but to put them here.
+ (char *)getSamplePath
{
	static char s[1024];
	
	const char *userPath = [[[NSBundle mainBundle] bundlePath] cStringUsingEncoding:NSASCIIStringEncoding];
	sprintf(s, "%s/samples", userPath);
	return (char *)s;
}

// app bundle path (skins, pngs, layout txt files - from the resources folder in Xcode)
+ (char *)getBundlePath
{
	const char *userPath = [[[NSBundle mainBundle] bundlePath] cStringUsingEncoding:NSASCIIStringEncoding];
	return (char *)userPath;
