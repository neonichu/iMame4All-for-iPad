...
// iControlPad stuff - Les Bird
int iControlPadState = 0;
int iControlPadCount = 0;
int iControlPadMode = 0;
int iControlPadButtonSet1 = 0;
int iControlPadButtonSet2 = 0;
int iControlPadNubX1 = 0;
int iControlPadNubY1 = 0;
int iControlPadNubX2 = 0;
int iControlPadNubY2 = 0;
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
  ...
  if(buttonIndex == 0 && wantExit )
  {
	...
	// iControlPad - Les Bird
	if (warnIcade && buttonIndex == 1)
	{
		iControlPadMode = 1;
	}
...
-(void)iCadeConnect
{
	...
	if(!iCadeUsed)
	{
		...
		if(first)
		{
			...
			// iControlPad - Les Bird
			warnAlert = [[UIAlertView alloc] initWithTitle:@"Connection!" 
												   message:[NSString stringWithFormat: @"iCade or iControlPad detected.\niCade may not be suitable for all games due to having only a single joystick. If you are using the iControlPad put it into KEYBOARD SPECIAL PACKET MODE (A + X) to use the analog sticks."]
												  delegate:self 
										 cancelButtonTitle:@"iCade"
										 otherButtonTitles:@"iControlPad", nil];
			//
			[warnAlert show];
			[warnAlert release];
		}
		else
		{
			...
			// iControlPad - Les Bird
			warnAlert = [[UIAlertView alloc] initWithTitle:nil 
												   message:[NSString stringWithFormat: @"\n\n\niCade or iControlPad connection?.\nPlease Wait..."]
												  delegate: nil 
										 cancelButtonTitle: nil 
										 otherButtonTitles: nil];
			//
...
-(BOOL)resignFirstResponder {
   ...
   {           	
           	if(iCadeUsed)
           	{
				...
				// iControlPad - Les Bird
	           	UIAlertView *warnAlert = [[UIAlertView alloc] initWithTitle:nil 
																	message:[NSString stringWithFormat: @"\n\n\niCade or iControlPad disconnection?.\nPlease Wait..."]
																   delegate: nil 
														  cancelButtonTitle: nil 
														  otherButtonTitles: nil];
				//
...
// iControlPad - Les Bird
- (void)interpretControlPadInputs
{
	//	NSLog(@"B%d,%d N%d,%d - %d,%d", iControlPadButtonSet1, iControlPadButtonSet2, iControlPadNubX1, iControlPadNubY1, iControlPadNubX2, iControlPadNubY2);
	
	gp2x_pad_status = 0;
	if ((iControlPadButtonSet1 & 0x20) != 0) // select button pressed
	{
		gp2x_pad_status |= GP2X_SELECT;
	}
	if ((iControlPadButtonSet1 & 0x10) != 0) // left trigger
	{
		gp2x_pad_status |= GP2X_L;
	}
	if ((iControlPadButtonSet1 & 0x08) != 0) // dpad down
	{
		gp2x_pad_status |= GP2X_DOWN;
	}
	if ((iControlPadButtonSet1 & 0x04) != 0) // dpad left
	{
		gp2x_pad_status |= GP2X_LEFT;
	}
	if ((iControlPadButtonSet1 & 0x02) != 0) // dpad right
	{
		gp2x_pad_status |= GP2X_RIGHT;
	}
	if ((iControlPadButtonSet1 & 0x01) != 0) // dpad up
	{
		gp2x_pad_status |= GP2X_UP;
	}
	if ((iControlPadButtonSet2 & 0x20) != 0) // right trigger
	{
		gp2x_pad_status |= GP2X_R;
	}
	if ((iControlPadButtonSet2 & 0x10) != 0) // B
	{
		gp2x_pad_status |= GP2X_B;
	}
	if ((iControlPadButtonSet2 & 0x08) != 0) // X
	{
		gp2x_pad_status |= GP2X_X;
	}
	if ((iControlPadButtonSet2 & 0x04) != 0) // A
	{
		gp2x_pad_status |= GP2X_A;
	}
	if ((iControlPadButtonSet2 & 0x02) != 0) // Y
	{
		gp2x_pad_status |= GP2X_Y;
	}
	if ((iControlPadButtonSet2 & 0x01) != 0) // start
	{
		gp2x_pad_status |= GP2X_START;
	}
	
	// left nub movements
	int dead_zone_value = 4 + (iOS_analogDeadZoneValue * 4);
	if (iControlPadNubX1 < -dead_zone_value)
	{
		gp2x_pad_status |= GP2X_LEFT;
	}
	else if (iControlPadNubX1 > dead_zone_value)
	{
		gp2x_pad_status |= GP2X_RIGHT;
	}
	if (iControlPadNubY1 < -dead_zone_value)
	{
		gp2x_pad_status |= GP2X_UP;
	}
	else if (iControlPadNubY1 > dead_zone_value)
	{
		gp2x_pad_status |= GP2X_DOWN;
	}
	// right nub movements
	if (iControlPadNubX2 < -dead_zone_value)
	{
		gp2x_pad_status |= GP2X_A;
	}
	else if (iControlPadNubX2 > dead_zone_value)
	{
		gp2x_pad_status |= GP2X_B;
	}
	if (iControlPadNubY2 < -dead_zone_value)
	{
		gp2x_pad_status |= GP2X_Y;
	}
	else if (iControlPadNubY2 > dead_zone_value)
	{
		gp2x_pad_status |= GP2X_X;
	}
	
	joy_analog_x[0] = ((float)iControlPadNubX1 / 32.0f);
	joy_analog_y[0] = ((float)iControlPadNubY1 / 32.0f);
	joy_analog_x[1] = ((float)iControlPadNubX2 / 32.0f);
	joy_analog_y[1] = ((float)iControlPadNubY2 / 32.0f);
	//	NSLog(@"%f,%f", joy_analog_x[0], joy_analog_y[0]);
}
//
...
- (void)insertText:(NSString *)theText 
{
	...
	if (iControlPadMode)
	{
		// put iControlPad in keyboard special packet mode (A + X)
		// interpret the data packets
		if (iControlPadCount == 0)
		{
			if (key == 'm') // button packet
			{
				iControlPadState = 1;
				iControlPadCount++;
			}
			else if (key == 'w') // nub packet
			{
				iControlPadState = 2;
				iControlPadCount++;
			}
		...
		else if (iControlPadState == 1) // read button data
		{
			if (iControlPadCount == 1)
			{
				iControlPadButtonSet1 = key - 32;
			}
			else
			{
				iControlPadButtonSet2 = key - 32;
				[self interpretControlPadInputs];
				iControlPadCount = -1;
				iControlPadState = 0;
			}
			iControlPadCount++;
		}
		else if (iControlPadState == 2) // read nub data
		{
			if (iControlPadCount == 1) // nub X1
			{
				iControlPadNubX1 = key - 64;
			}
			else if (iControlPadCount == 2) // nub y1
			{
				iControlPadNubY1 = key - 64;
			}
			else if (iControlPadCount == 3) // nub x2
			{
				iControlPadNubX2 = key - 64;
			}
			else if (iControlPadCount == 4) // nub y2
			{
				iControlPadNubY2 = key - 64;
				[self interpretControlPadInputs];
				iControlPadCount = -1;
				iControlPadState = 0;
			}
